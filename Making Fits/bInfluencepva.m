function [p, d, Dstar] = bInfluencepva(data,Tboundaries,Influencecdf,file,varargin)

% bInfluencepva calculates the p-value for the given power-law fit to some data.
...
% bInfluencepva comes with ABSOLUTELY NO WARRANTY
...
while i<=length(varargin)
    ...
    if ~argok,
        disp(['(bInfluencepva) Ignoring invalid argument #' num2str(i+2)]); 
    end
    i=i+1;
end
...
if isequal(fix(reshape(data,numel(data),1)),reshape(data,numel(data),1))==0
    fprintf('(Influencepva) Error: Vector h should be an integer vector.\n');
    return;
end

% 2. h must be non-negative
if ~isempty(find(reshape(data,numel(data),1)<0, 1))
    fprintf('(bInfluencepva) Error: Vector h should be non-negative.\n');
    return;
end
...
if numel(Tboundaries)~=(numel(data(1,:))+1)
    fprintf('(bInfluencepva) Error: Incorrect number of elements in either boundaries or h.\n');
    return;
end

% 4. Need atleast 2 bins to work with.
if numel(data(1,:))<2
    fprintf('(bInfluencepva) Error: I need atleast 2 bins to make this work.\n');
    return;
end

% 5. Checking number of repititons
if ~isempty(reps) && (~isscalar(reps) || reps<2),
	fprintf('(bInfluencepva) Error: ''reps'' argument must be a positive value > 1; using default.\n');
    reps = 1000;
end;
...
InfluenceCDF = GetInfluenceCDF(data,Tboundaries,Influencecdf,file);
Dstar=ks2d1sHist(data,InfluenceCDF);
N = length(data(:,1)) - 1;
% ---------------------------------------------------------------
...
parfor i=1:reps
    ...
    % create random numbers from Influence
    ...
    for j = 1:N+1
        ...
        % NOTE:InfluenceCDF(j,end) < 1, thus we need to normalize
        newCDF = InfluenceCDF(j,:)./InfluenceCDF(j,end);
	%disp(newCDF);
	% suspicion: CDF not well defined, but hv > 0
        newCDF(newCDF > 1) = 1;
        newCDF(:,1) = 0;
        newCDF(newCDF < 0) = 0;
    ...
    bootCDF = GetInfluenceCDF(zq,Tboundaries,Influencecdf,file);
